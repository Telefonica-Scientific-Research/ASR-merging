#!/usr/bin/env python3
"""Whisper-Turbo training/evaluation router for VC, OpenSLR, and MLC datasets.

This script mirrors the notebook's Whisper-Turbo configuration style:
...
Examples:
    python -m asr_merging.whisper_turbo_train_router \
...
class Config:
    ...
    # Paths (aligned with Whisper-Turbo notebook cache roots)
    processed_cv_cache_dir: Path = Path("data/cache/processed_cv_datasets/whisper_baseline")
    openslr_cache_dir: Path = Path("data/cache/processed_openslr")
    mlc_cache_dir: Path = Path("data/cache/whisper")

    # Whisper generation controls
    ...
    config_overrides.update(
        {
            ...
            "whisper_language": merged["whisper_language"],
            "whisper_task": merged["whisper_task"],
        }
    )
...
class WhisperTurboCollator:
    """Collator for Whisper processed datasets with `input_features` and `labels`."""

    def __init__(self, processor):
        self.processor = processor

    def __call__(self, features: List[dict]) -> Dict[str, torch.Tensor]:
        feat_items = [{"input_features": np.asarray(f["input_features"], dtype=np.float32)} for f in features]
        padded_feats = self.processor.feature_extractor.pad(feat_items, return_tensors="pt")

        label_items = []
        for f in features:
            label_items.append({"input_ids": [int(x) for x in f["labels"] if int(x) != -100]})
        padded_labels = self.processor.tokenizer.pad(label_items, return_tensors="pt")
        labels = padded_labels["input_ids"]
        if self.processor.tokenizer.pad_token_id is not None:
            labels[labels == self.processor.tokenizer.pad_token_id] = -100

        input_features = padded_feats["input_features"]
        return {"input_features": input_features, "labels": labels}


def load_whisper_base_model(config: Config) -> WhisperForConditionalGeneration:
    ...
    model = WhisperForConditionalGeneration.from_pretrained(config.model_id, **load_kwargs)
...
def _ensure_processed(ds: Dataset, split_name: str) -> Dataset:
    ...
    if not {"input_features", "labels"}.issubset(cols):
        raise RuntimeError(
            f"Split '{split_name}' is not Whisper processed. Required columns: ['input_features','labels'], got {ds.column_names}"
        )
    return ds
...
def recover_vc_splits(config: Config, language: str) -> Dict[str, Tuple[Dataset, List[str]]]:
    ...
    if not root.exists():
        raise FileNotFoundError(f"VC Whisper cache not found: {root}")
...
def recover_openslr_splits(config: Config) -> Dict[str, Tuple[Dataset, List[str]]]:
    ...
    if not root.exists():
        raise FileNotFoundError(f"OpenSLR cache root not found: {root}")

    candidates = sorted([p for p in root.glob("pipe=whisper__*") if p.is_dir()], reverse=True)
    if not candidates:
        candidates = sorted([p for p in root.glob("*") if p.is_dir()], reverse=True)
    if not candidates:
        raise FileNotFoundError(f"No Whisper OpenSLR cache found under: {root}")
...
def _build_whisper_generate_kwargs(language: str, task: str, max_new_tokens: int) -> Dict:
    return {
        "language": language,
        "task": task,
...
def whisper_generate_predictions(
    model,
    dataset: Dataset,
    processor,
    ...
    for start in range(0, len(dataset), batch_size):
        ...
        feat_items = [{"input_features": np.asarray(item, dtype=np.float32)} for item in batch["input_features"]]
        padded = processor.feature_extractor.pad(feat_items, return_tensors="pt")
        feats = padded["input_features"].to(model.device, dtype=torch.float32)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Whisper-Turbo dataset router (VC/MLC/OpenSLR)", argument_default=argparse.SUPPRESS)
    ...
    p.add_argument("--whisper-language", help="Whisper generation language (e.g. english, serbian, spanish).")
    p.add_argument("--whisper-task", help="Whisper generation task (typically transcribe).")
    return p.parse_args()
...
def main() -> None:
    ...
    if merged["output_dir"]:
        ...
        exp_prefix = merged["experiment_name"] or f"whisper_turbo_router_{merged['source']}"
        output_dir = Path(merged["output_root"]) / f"{exp_prefix}_{ts}"
    else:
        output_dir = Path(merged["output_root"]) / "whisper_turbo_router_run"
    ...
    if run_train:
        ...
        if merged["model_mode"] == "adapter" and merged["adapter_path"]:
            base = load_whisper_base_model(config)
            ...
            trainer, active_model = train_whisper_turbo_lora(
                config=config,
                processor=processor,
                train_dataset=train_ds,
    ...
    if run_eval:
        ...
        preds = whisper_generate_predictions(
            model=active_model,
            dataset=eval_ds,
            processor=processor,
            whisper_language=merged["whisper_language"],
