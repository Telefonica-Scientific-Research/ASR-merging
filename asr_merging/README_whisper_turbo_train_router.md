# Whisper-Turbo Training Router

This guide explains how to use `asr_merging/whisper_turbo_train_router.py`.

The script follows the workflow in `notebooks/whisper-turbo-FT-QLORA.ipynb` and keeps the same interface pattern as Voxtral and Seamless routers:
...
python -m asr_merging.whisper_turbo_train_router \
  ...
  [--language <sr|mt|da>] \
  [--whisper-language <english|serbian|spanish|...>] \
...
## 3) Cache roots aligned with the Whisper-Turbo notebook

- VC cache root: `data/cache/processed_cv_datasets/whisper_baseline/<lang>/`
- OpenSLR cache root: `data/cache/processed_openslr/`
- MLC cache root: `data/cache/whisper/mlc_slm_*/`
...
- `configuration/whisper_turbo_mlc_train_eval_baseline.json`
- `configuration/whisper_turbo_mlc_eval_adapter.json`
- `configuration/whisper_turbo_vc_sr_train_baseline.json`

Example with config + CLI override:

```bash
python -m asr_merging.whisper_turbo_train_router \
  --config-json configuration/whisper_turbo_mlc_train_eval_baseline.json \
  --learning-rate 3e-5
```
...
## 7) Whisper generation controls

- `--whisper-language`: generation language hint (e.g. `english`, `serbian`, `spanish`)
- `--whisper-task`: generation task (`transcribe` by default)
