#!/bin/bash
#SBATCH --job-name=voc_n129_frz_bal
#SBATCH --output=./slurm_logs/voc_n129_frz_bal_%j.out
#SBATCH --error=./slurm_logs/voc_n129_frz_bal_%j.err
...
echo "ASR-merging Voxtral MCQ — 129-file NLLB, freeze enc+conn, chat_tmpl, bs=16"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --test-jsonl data/mlc26_task2/eval_21files_en_only_balanced.jsonl \
  ...
  --experiment-name voxtral_mcq_nllb129_frz_enc_conn_chat_tmpl_balanced_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ nllb129 frz-enc-conn chat_tmpl bs=16 training completed successfully'
