#!/bin/bash
#SBATCH --job-name=voc_bs16_nllb_up
#SBATCH --output=./slurm_logs/voc_bs16_nllb_up_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_nllb_up_%j.err
...
# CL Phase 2 (bs16 base) — Option B: EN×1 + NLLB×8 (1,350 rec, 40,500q)
#
...
#   - 1350 records, 40500 questions, EN share 11%
#   - Steps (1 epoch, batch=16): ~2531
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_b_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option B"
echo "=========================================="
...
# 40500q / 16 = ~2531 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16_b_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option B fine-tuning completed successfully'
trap - EXIT
"
