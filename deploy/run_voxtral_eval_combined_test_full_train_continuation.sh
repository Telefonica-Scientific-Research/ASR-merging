#!/bin/bash
#SBATCH --job-name=voxtral_eval_mlc25test_mlc26dev_full_cont
#SBATCH --output=./slurm_logs/voxtral_eval_mlc25test_mlc26dev_full_cont_%j.out
#SBATCH --error=./slurm_logs/voxtral_eval_mlc25test_mlc26dev_full_cont_%j.err
...
torchrun --standalone --nproc_per_node=1 --master_port \${MASTER_PORT} \
  --log-dir \${TORCHRUN_LOG_DIR} --redirects 3 --tee 1 --local-ranks-filter 0 \
  -m asr_merging.voxtral_train_router \
  --config-json configuration/mlc26_eval_mlc26_dev_full_train_continuation.json
