#!/bin/bash
#SBATCH --job-name=voxtral_connector_only
#SBATCH --output=./slurm_logs/voxtral_connector_only_%j.out
#SBATCH --error=./slurm_logs/voxtral_connector_only_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — connector-only (encoder+LLM frozen)"
echo "=========================================="
...
# Connector-only: --freeze-encoder freezes audio_tower, --freeze-llm freezes LLM
# Only multi_modal_projector / connector params are trained
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_connector_only_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ connector-only 4-GPU training completed successfully'
