#!/bin/bash
#SBATCH --job-name=voxtral_cl_nlsf_shuf
#SBATCH --output=./slurm_logs/voxtral_cl_nlsf_shuf_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_nlsf_shuf_%j.err
...
# Curriculum Learning Phase 2 — Option C_shuffled: nllb_safe + option shuffling.
#
# Identical to Option C (EN×2 + NLLB×8) but with answer options permuted for
# every non-EN record (1,426/1,500 records).  Uses the same deterministic
# per-question shuffle as B_shuf / A_shuf.
#
# Motivation: C had the 2nd-best pilot eval_loss (0.0480 vs A's 0.0472).
# Shuffling breaks the per-question positional memorization that arises when
# the same question appears 8× in 8 languages with the same correct position.
#
# Comparison:
#   C          nllb_safe           EN×2 + NLLB×8             original option order
#   C_shuffled nllb_safe_shuffled  EN×2 + NLLB×8 (shuffled)  anti-memorization  ← this script
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_safe_shuffled.jsonl
#   - 1,500 records  (EN×2=74 unchanged + NLLB×8=1,426 shuffled)
#   - 45,000 questions
#   - Answer dist: A=28% B=29% C=22% D=21% (vs original B=41%)
#   - EN share: 20%
#   - Steps (1 epoch, batch=32): ~1,406
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/.../final_model \
#   sbatch deploy/run_voxtral_mcq_cl_nllb_safe_shuffled_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: nllb_safe + option shuffling (C_shuffled)"
echo "=========================================="
...
# 45,000q / 32 = ~1,406 steps for 1 epoch (early stopping patience=5)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_safe_shuffled.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_nllb_safe_shuf_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 nllb_safe shuffled fine-tuning completed successfully'
trap - EXIT
"
