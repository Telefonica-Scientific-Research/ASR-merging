#!/bin/bash
#SBATCH --job-name=voc_bs16_fullmix
#SBATCH --output=./slurm_logs/voc_bs16_fullmix_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_fullmix_%j.err
...
# CL Phase 2 (bs16 base) — Option D: EN×1 + native + NLLB×8 (1,463 rec, 43,890q)
#
...
#   - 1463 records, 43890 questions, EN share 10%
#   - Steps (1 epoch, batch=16): ~2743
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_d_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option D"
echo "=========================================="
...
# 43890q / 16 = ~2743 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --no-use-bf16 \
  --use-fp16 \
  --output-dir experiments/voxtral_mcq_cl_bs16_d_ft_4gpu_20260621_123554 \
  --resume-from-checkpoint checkpoint-750 \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option D fine-tuning completed successfully'
trap - EXIT
"
