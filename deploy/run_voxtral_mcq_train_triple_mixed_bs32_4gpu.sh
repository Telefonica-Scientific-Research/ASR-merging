#!/bin/bash
#SBATCH --job-name=voxtral_triple_mixed_bs32
#SBATCH --output=./slurm_logs/voxtral_triple_mixed_bs32_%j.out
#SBATCH --error=./slurm_logs/voxtral_triple_mixed_bs32_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — triple-mix: original + NLLB + Qwen3 translated questions"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_triple_mixed.jsonl \
  --eval-jsonl data/mlc26_task2/organisers_balanced_triple_mixed.jsonl \
  --test-jsonl data/mlc26_task2/organisers_triple_mixed.jsonl \
  ...
  --experiment-name voxtral_mcq_triple_mixed_bs32_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ triple-mix (orig+nllb+qwen3) 4-GPU training completed successfully'
