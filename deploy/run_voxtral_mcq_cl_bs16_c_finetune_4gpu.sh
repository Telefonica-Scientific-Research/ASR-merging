#!/bin/bash
#SBATCH --job-name=voc_bs16_nllb_sf
#SBATCH --output=./slurm_logs/voc_bs16_nllb_sf_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_nllb_sf_%j.err
...
# CL Phase 2 (bs16 base) — Option C: EN×2 + NLLB×8 (1,500 rec, 45,000q)
#
...
#   - 1500 records, 45000 questions, EN share 20%
#   - Steps (1 epoch, batch=16): ~2812
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_c_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option C"
echo "=========================================="
...
# 45000q / 16 = ~2812 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16_c_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option C fine-tuning completed successfully'
trap - EXIT
"
