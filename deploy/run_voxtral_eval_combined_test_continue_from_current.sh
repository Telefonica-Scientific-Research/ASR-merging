#!/bin/bash
#SBATCH --job-name=voxtral_eval_mlc25test_mlc26dev_continue
#SBATCH --output=./slurm_logs/voxtral_eval_mlc25test_mlc26dev_continue_%j.out
#SBATCH --error=./slurm_logs/voxtral_eval_mlc25test_mlc26dev_continue_%j.err
...
echo "ASR-merging Voxtral Router Eval (mlc25 test + mlc26 dev, sequential)"
echo "Adapter: mlc25_mlc26_continue_from_current"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=1 --master_port \${MASTER_PORT} \
  ...
  --config-json configuration/mlc26_eval_mlc25_test_continue_from_current.json

echo '--- mlc25 test eval done; starting mlc26 dev eval ---'

torchrun --standalone --nproc_per_node=1 --master_port \${MASTER_PORT} \
  --log-dir \${TORCHRUN_LOG_DIR} --redirects 3 --tee 1 --local-ranks-filter 0 \
  -m asr_merging.voxtral_train_router \
  --config-json configuration/mlc26_eval_mlc26_dev_continue_from_current.json
