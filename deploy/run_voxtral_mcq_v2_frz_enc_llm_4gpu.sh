#!/bin/bash
#SBATCH --job-name=voxtral_v2_frz_enc_llm
#SBATCH --output=./slurm_logs/voxtral_v2_frz_enc_llm_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_frz_enc_llm_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — v2 mixed | freeze encoder+LLM | connector only"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_frz_enc_llm_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_frz_enc_llm_4gpu completed successfully'
