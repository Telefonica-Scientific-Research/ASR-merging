#!/bin/bash
#SBATCH --job-name=voxtral_cl_nllb_up
#SBATCH --output=./slurm_logs/voxtral_cl_nllb_up_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_nllb_up_%j.err
...
# Curriculum Learning Phase 2 — NLLB upsampling variant.
#
# Trains on the same 150 EN audio files but with questions upsampled across
# 9 languages (EN + 8 NLLB-translated variants): por/fra/spa/rus/vie/tur/tgl/deu.
# This injects CROSS-LINGUAL signal (EN audio + non-EN questions), which is the
# dominant format in the MLC-SLM challenge (~52% cross-lingual).
#
# Contrast with run_voxtral_mcq_cl_native_finetune_4gpu.sh (Option A):
#   Option A: EN audio+EN q  +  native audio+native q  → same-language pairs
#   Option B: EN audio+EN q  +  EN audio+NLLB-lang q   → cross-language pairs  ← this script
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_upsample.jsonl
#   - 1,350 records  (150 files × 9 languages)
#   - 40,500 questions  (all cross-lingual except the 4,500 EN ones)
#   - EN share: 11.1%  (vs 56.7% in Option A — less EN replay, more forgetting risk)
...
#   sbatch deploy/run_voxtral_mcq_cl_nllb_upsample_finetune_4gpu.sh
#
# Config vs Round 6:
#   enc_conn LR : 1e-5  →  5e-6  (halved)
#   llm LR      : 5e-5  →  2e-5  (halved)
#   epochs      : 3     →  1     (1 epoch already = ~1,265 steps due to dataset size)
#   patience    : 5     →  5     (kept at 5 — EN share is low, model needs room to adapt)
...
echo "Voxtral MCQ — CL Phase 2: NLLB cross-lingual upsampling"
echo "=========================================="
...
# 40,500q / 32 = ~1,265 steps for 1 epoch (early stopping with patience=5 expected)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_upsample.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_nllb_up_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 NLLB upsampling fine-tuning completed successfully'
trap - EXIT
"
