#!/bin/bash
#SBATCH --job-name=voxtral_noen_mbalw_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_noen_mbalw_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_noen_mbalw_trx_4gpu_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — R6-base + 150-file mixed weighted-loss + transcript"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_150_balanced_noen_weighted.jsonl \
  --eval-jsonl data/mlc26_task2/organisers_balanced_mixed_weighted.jsonl \
  ...
  --experiment-name voxtral_mcq_noen_mixed_bal_weighted_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ 150-file mixed weighted-loss + transcript 4-GPU training completed successfully'
