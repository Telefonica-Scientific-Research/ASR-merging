#!/bin/bash
#SBATCH --job-name=voxtral_v2r_frz_enc_conn
#SBATCH --output=./slurm_logs/voxtral_v2_frz_enc_conn_remap_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_frz_enc_conn_remap_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_frz_enc_conn_remap_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_frz_enc_conn_4gpu remap completed successfully'
