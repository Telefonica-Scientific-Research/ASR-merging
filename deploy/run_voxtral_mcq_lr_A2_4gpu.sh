#!/bin/bash
#SBATCH --job-name=voxtral_mcq_lr_A2
#SBATCH --output=./slurm_logs/voxtral_mcq_lr_A2_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_lr_A2_%j.err
...
echo "ASR-merging Voxtral MCQ LR A2 enc=1e-4 llm=5e-5 4-GPU"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --eval-batch-size 1 \
  --grad-accum-steps 4 \
  --encoder-connector-learning-rate 1e-4 \
  --llm-learning-rate 5e-5 \
  --no-use-bf16 \
  --use-fp16 \
  --output-root experiments \
  --experiment-name voxtral_mcq_lr_A2 \
  --prompt-language en \
