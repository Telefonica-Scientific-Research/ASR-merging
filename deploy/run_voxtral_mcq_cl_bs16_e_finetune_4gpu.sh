#!/bin/bash
#SBATCH --job-name=voc_bs16_fms
#SBATCH --output=./slurm_logs/voc_bs16_fms_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_fms_%j.err
...
# CL Phase 2 (bs16 base) — Option E: EN×2 + native + NLLB×8 (1,613 rec, 48,390q)
#
...
#   - 1613 records, 48390 questions, EN share 19%
#   - Steps (1 epoch, batch=16): ~3024
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_e_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option E"
echo "=========================================="
...
# 48390q / 16 = ~3024 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16_e_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option E fine-tuning completed successfully'
trap - EXIT
"
