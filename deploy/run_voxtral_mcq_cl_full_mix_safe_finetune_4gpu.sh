#!/bin/bash
#SBATCH --job-name=voxtral_cl_fms
#SBATCH --output=./slurm_logs/voxtral_cl_fms_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_fms_%j.err
...
# Curriculum Learning Phase 2 — Option E: full mix safe (EN×2 + native non-EN + NLLB×8).
#
# Kitchen-sink dataset: every signal type plus extra EN replay for forgetting safety.
...
#   - EN anchor ×2 to keep English performance stable
#
...
#   E  full_mix_safe  EN×2 + native non-EN + NLLB×8         both,            19% EN  ← this script
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_full_mix_safe.jsonl
#   - 1,613 records  (EN×2=300 + native=113 + NLLB×8=1200)
#   - 48,390 questions
#   - EN share: 19%
...
#   sbatch deploy/run_voxtral_mcq_cl_full_mix_safe_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: full mix safe (EN×2 + native + NLLB×8)"
echo "=========================================="
...
# 48,390q / 32 = ~1,512 steps for 1 epoch (early stopping with patience=5 expected)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_full_mix_safe.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_full_mix_safe_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 full mix safe fine-tuning completed successfully'
trap - EXIT
"
