#!/bin/bash
#SBATCH --job-name=voc_bs16h_nup
#SBATCH --output=./slurm_logs/voc_bs16h_nup_%j.out
#SBATCH --error=./slurm_logs/voc_bs16h_nup_%j.err
...
# CL Phase 2 (bs16 halved-LR) — Option B: EN×1 + NLLB×8
#
...
echo "Voxtral MCQ — CL bs16-halfLR Phase 2: Option B"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16h_b_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16-halfLR Phase 2 Option B completed successfully'
trap - EXIT
"
