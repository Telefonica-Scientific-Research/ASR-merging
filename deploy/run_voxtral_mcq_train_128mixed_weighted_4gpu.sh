#!/bin/bash
#SBATCH --job-name=voxtral_128mw_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_128mw_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_128mw_trx_4gpu_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_128files_mixed_weighted.jsonl \
  ...
  --experiment-name voxtral_mcq_128mixed_weighted_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
