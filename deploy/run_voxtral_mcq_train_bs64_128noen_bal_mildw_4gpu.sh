#!/bin/bash
#SBATCH --job-name=voxtral_bs64_128noen_bal_mildw_4gpu
#SBATCH --output=./slurm_logs/voxtral_bs64_128noen_bal_mildw_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_bs64_128noen_bal_mildw_4gpu_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_128_balanced_noen_mildw.jsonl \
  ...
  --experiment-name voxtral_mcq_bs64_128noen_bal_mildw_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
