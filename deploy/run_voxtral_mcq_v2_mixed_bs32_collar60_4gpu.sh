#!/bin/bash
#SBATCH --job-name=voxtral_v2_col60
#SBATCH --output=./slurm_logs/voxtral_v2_mixed_bs32_collar60_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_mixed_bs32_collar60_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — v2 mixed | 124-file held-out eval | bs32 | collar=60s + remap"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_bs32_collar60_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
  --crop-from-question-refs \
  --remap-timestamps-after-crop \
  --crop-collar-seconds 60 \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_bs32_collar60_4gpu completed successfully'
