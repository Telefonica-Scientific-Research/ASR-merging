#!/bin/bash
#SBATCH --job-name=voxtral_128noen_bw_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_128noen_bw_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_128noen_bw_trx_4gpu_%j.err
...
# R6-base + 128-file non-overlap mixed dataset + transcript augmentation
# ────────────────────────────────────────────────────────────────────────
# TRAIN: train_128files_mixed_balanced.jsonl
#   128 audio files (129 minus 1 overlap with eval), 7680 questions
#   = NLLB-balanced English questions + original cross-lingual questions
#   True held-out eval: eval audio files NOT seen during training
#
# EVAL:  eval_21files_mixed_balanced.jsonl
#   21 audio files, 1260 questions (NLLB + cross-lingual)
#   No audio overlap with train set
#
# TEST:  organisers_mixed.jsonl (full 149-file challenge set)
#
# min_generalization_gap strategy + patience=0 → keep ALL checkpoints
# Transcripts injected for all audio files in data/transcripts/

set -euo pipefail

echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "ASR-merging Voxtral MCQ Train 4-GPU — R6-base + 128-file non-overlap mixed weighted-loss + transcript"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_128_balanced_noen_weighted.jsonl \
  --eval-jsonl data/mlc26_task2/eval_21files_mixed_weighted.jsonl \
  ...
  --experiment-name voxtral_mcq_128noen_bal_weighted_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ 128-file non-overlap mixed weighted-loss + transcript 4-GPU training completed successfully'
