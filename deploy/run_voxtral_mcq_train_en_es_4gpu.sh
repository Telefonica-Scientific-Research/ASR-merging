#!/bin/bash
#SBATCH --job-name=voxtral_mcq_en_es_4gpu
#SBATCH --output=./slurm_logs/voxtral_mcq_en_es_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_en_es_4gpu_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — EN+ES questions ablation"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_en_es.jsonl \
  --eval-jsonl data/mlc26_task2/organisers_balanced_en_es.jsonl \
  --test-jsonl data/mlc26_task2/organisers_en_es.jsonl \
  ...
  --experiment-name voxtral_mcq_en_es_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ EN+ES ablation 4-GPU training completed successfully'
