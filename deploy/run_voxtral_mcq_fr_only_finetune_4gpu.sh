#!/bin/bash
#SBATCH --job-name=voc_fr_only
#SBATCH --output=./slurm_logs/voc_fr_only_%j.out
#SBATCH --error=./slurm_logs/voc_fr_only_%j.err
...
# Per-language LoRA from OOTB Voxtral — FR (French, French_Canada)
#
...
# Train: train_124files_fr_only.jsonl — 15 records, 450 questions
# Eval:  eval_26files_fr_only.jsonl  — 2 records, 60 questions (zero overlap)
# Steps: 450q / batch16 = ~28 steps/epoch; 10 epochs max
set -euo pipefail
echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "Voxtral MCQ — FR-only fine-tuning (OOTB base)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_124files_fr_only.jsonl \
  --eval-jsonl  data/mlc26_task2/eval_26files_fr_only.jsonl \
  --test-jsonl  data/mlc26_task2/eval_26files_fr_only.jsonl \
  ...
  --experiment-name voxtral_mcq_fr_only_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
  ...
  2>&1
echo 'FR-only fine-tuning completed successfully'
trap - EXIT
"
