#!/bin/bash
#SBATCH --job-name=voxtral_mcq_split_balanced_4gpu
#SBATCH --output=./slurm_logs/voxtral_mcq_split_balanced_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_split_balanced_4gpu_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — audio-file split"
echo "=========================================="
...
# Train: 129 audio files (3870 questions, label-balanced: A≈B≈C≈D≈25%).
# Eval:   21 files (630 questions, 1 per language) used for BOTH:
#   --eval-jsonl  → step-level eval loss, drives early stopping
#   --test-jsonl  → final MCQ accuracy reported after training
# Effective batch size: 4 GPUs x 1 per-GPU batch x 4 grad_accum = 16.
# Early stopping patience=3 on eval_loss; up to 5 epochs.
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_train_audio_split_balanced.jsonl \
  --eval-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_eval_audio_split.jsonl \
  --test-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_eval_audio_split.jsonl \
  ...
  --experiment-name voxtral_mcq_audio_split_balanced_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ audio-split 4-GPU training completed successfully'
