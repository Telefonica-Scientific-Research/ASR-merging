#!/bin/bash
#SBATCH --job-name=voxtral_cl_fms_shuf
#SBATCH --output=./slurm_logs/voxtral_cl_fms_shuf_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_fms_shuf_%j.err
...
# Curriculum Learning Phase 2 — Option E_shuffled: full mix safe + option shuffling.
#
# Kitchen-sink dataset (EN×2 + native + NLLB×8) with answer options permuted for
# every non-EN NLLB-translated record.  EN and native records keep their original
# option order.  Each NLLB language copy uses a deterministic per-question shuffle
# keyed on (question_id, language), breaking the consistent answer position across
# language copies of the same question.
#
# This is the most comprehensive anti-memorization experiment: all signal types
# (EN anchor, native same-lang, NLLB cross-lang) plus positional de-correlation.
#
# Experiment matrix for CL Phase 2 (shuffled variants):
#   B          nllb_up          EN×1 + NLLB×8                original option order
#   B_shuffled nllb_up_shuffled EN×1 + NLLB×8 (shuffled)    anti-memorization
#   E          full_mix_safe    EN×2 + native + NLLB×8       original option order
#   E_shuffled fms_shuffled     EN×2 + native + NLLB×8 (sh.) anti-memorization  ← this script
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_full_mix_safe_shuffled.jsonl
#   - 1,613 records  (EN×2=300 unchanged + native=113 unchanged + NLLB×8=1200 shuffled)
#   - 48,390 questions
#   - EN share: 19%
#   - Steps (1 epoch, batch=32): ~1,512
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/.../final_model \
#   sbatch deploy/run_voxtral_mcq_cl_full_mix_safe_shuffled_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: full mix safe + option shuffling (E_shuffled)"
echo "=========================================="
...
# 48,390q / 32 = ~1,512 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_full_mix_safe_shuffled.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_fms_shuf_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 full mix safe shuffled fine-tuning completed successfully'
trap - EXIT
"
