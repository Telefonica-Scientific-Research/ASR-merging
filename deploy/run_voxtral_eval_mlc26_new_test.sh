#!/bin/bash
#SBATCH --job-name=voxtral_wer_mlc26_new
#SBATCH --output=./slurm_logs/voxtral_wer_mlc26_new_%j.out
#SBATCH --error=./slurm_logs/voxtral_wer_mlc26_new_%j.err
...
export KEEP_ALIVE_ON_FAILURE=0
...
torchrun --standalone --nproc_per_node=1 --master_port \${MASTER_PORT} \
  ...
  -m asr_merging.voxtral_eval_router \
