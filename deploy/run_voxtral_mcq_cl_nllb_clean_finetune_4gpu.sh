#!/bin/bash
#SBATCH --job-name=voxtral_cl_nllb_cln
#SBATCH --output=./slurm_logs/voxtral_cl_nllb_cln_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_nllb_cln_%j.err
...
# Curriculum Learning Phase 2 — Option F: NLLB clean (EN-origin audio only).
#
# "Safe NLLB" variant: uses ONLY the 37 audio files whose original questions were
# already in English, so the NLLB translation path is strictly EN → target_lang.
# Discards the 113 non-EN-audio records from en_to_*, which would go through
# native_lang → EN → target_lang (round-trip, lower fidelity).
#
...
#   F  nllb_clean          EN×1 + NLLB_clean×8 (37 files only)  clean cross-lang  ← this script
#   G  nllb_clean_native   EN×1 + NLLB_clean×8 + native         clean cross-lang + same-lang
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_clean.jsonl
#   - 333 records  (EN×1=37 + NLLB-clean×8=296)
#   - 9,990 questions
#   - EN share: 11%
#   - Steps (1 epoch, batch=32): ~312  — fast run, similar size to Option A
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/voxtral_mcq_nllb150_r32a64_duallr_chat_tmpl_4gpu_<ts>/final_model \
#   sbatch deploy/run_voxtral_mcq_cl_nllb_clean_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: NLLB clean (EN-origin only, 37 files × 9 langs)"
echo "=========================================="
...
# 9,990q / 32 = ~312 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_clean.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_nllb_clean_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 NLLB clean fine-tuning completed successfully'
trap - EXIT
"
