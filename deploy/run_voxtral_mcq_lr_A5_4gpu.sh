#!/bin/bash
#SBATCH --job-name=voxtral_mcq_lr_A5
#SBATCH --output=./slurm_logs/voxtral_mcq_lr_A5_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_lr_A5_%j.err
...
echo "ASR-merging Voxtral MCQ LR A5 enc=5e-5 llm=1e-5 4-GPU"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_lr_A5 \
  --prompt-language en \
