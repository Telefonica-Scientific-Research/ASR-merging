#!/bin/bash
#SBATCH --job-name=voc_n129_bal_all
#SBATCH --output=./slurm_logs/voc_n129_bal_all_%j.out
#SBATCH --error=./slurm_logs/voc_n129_bal_all_%j.err
...
# Round 6 — nllb129 balanced all-component:
#   Data   : all 150-file NLLB-EN (same as nllb_bs16 that scored 0.7382 on challenge)
#   LoRA   : r=32, alpha=64  (§7.7.1: +3pp over default r=16/a=32)
#   LR     : enc_conn=1e-5 / llm=5e-5  (§7.7.2 A3: +1.7pp over uniform 5e-5)
#   Format : --use-chat-template-for-training  (§7.4.4: +0.63pp)
#   bs=32  : grad-accum=8, 4GPU×1×8  (§7.1.3: bs32 > bs16 uniformly)
#   collar=30s, crop=300s, no remap  (§7.3.0/§7.3.4)
#   patience=5  (r=32 converges slower, needs more patience than default 3)
#   NO freeze-encoder-connector  (diff LR already regularizes enc_conn; A3 > B1)

set -euo pipefail

echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "ASR-merging Voxtral MCQ Train 4-GPU — Round 6 (r32/a64 + dual-LR + chat_tmpl)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_129files_nllb_balanced.jsonl \
  --eval-jsonl data/mlc26_task2/eval_21files_en_only_balanced.jsonl \
  ...
  --lora-r 32 \
  --lora-alpha 64 \
  --encoder-connector-learning-rate 1e-5 \
  --llm-learning-rate 5e-5 \
  --use-chat-template-for-training \
  --no-use-bf16 \
  --use-fp16 \
  --output-root experiments \
  --experiment-name voxtral_mcq_nllb129_r32a64_duallr_chat_tmpl_balanced_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ Round 6 (r32/a64 + dual-LR + chat_tmpl) training completed successfully'
