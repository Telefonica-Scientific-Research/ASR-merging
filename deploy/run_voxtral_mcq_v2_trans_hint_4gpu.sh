#!/bin/bash
#SBATCH --job-name=voxtral_v2_trans_hint
#SBATCH --output=./slurm_logs/voxtral_v2_trans_hint_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_trans_hint_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — v2 mixed | use-transcription-hint-format (lang:en[TRANSCRIBE] inside [INST])"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_trans_hint_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_trans_hint_4gpu completed successfully'
