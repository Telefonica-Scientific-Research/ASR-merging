#!/bin/bash
#SBATCH --job-name=voxtral_v2_nllb_frz_chat
#SBATCH --output=./slurm_logs/voxtral_v2_nllb_frz_chat_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_nllb_frz_chat_%j.err
...
# Data: train_129files_nllb.jsonl — NLLB English (all 150 files, 1 per language in test)
#   Train: 129 files × 30q = 3870q, all English (NLLB cross-lang + EN+EN pass-through)
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_129files_nllb.jsonl \
  ...
  --experiment-name voxtral_mcq_v2_nllb_frz_enc_conn_chat_tmpl_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_nllb_frz_enc_conn_chat_tmpl_4gpu completed successfully'
