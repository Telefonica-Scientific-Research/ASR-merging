#!/bin/bash
#SBATCH --job-name=voxtral_v2_asr_base
#SBATCH --output=./slurm_logs/voxtral_v2_asr_base_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_asr_base_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — v2 mixed | ASR-merged base + MCQ LoRA (mlc26 full_train continuation)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_124files_mixed.jsonl \
  ...
  --experiment-name voxtral_mcq_v2_mixed_asr_base_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
  ...
  --base-adapter-path experiments/mlc25_mlc26_continue_4gpu_fixed_bothsides_20260530_120641/final_model \
  --use-transcription-hint-format \
  2>&1

echo 'Voxtral MCQ voxtral_mcq_v2_mixed_asr_base_4gpu completed successfully'
