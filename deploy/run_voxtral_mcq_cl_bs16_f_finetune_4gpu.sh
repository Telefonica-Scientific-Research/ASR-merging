#!/bin/bash
#SBATCH --job-name=voc_bs16_nlcln
#SBATCH --output=./slurm_logs/voc_bs16_nlcln_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_nlcln_%j.err
...
# CL Phase 2 (bs16 base) — Option F: EN-origin×1 + NLLB-clean×8 (333 rec, 9,990q)
#
...
#   - 333 records, 9990 questions, EN share 11%
#   - Steps (1 epoch, batch=16): ~624
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_f_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option F"
echo "=========================================="
...
# 9990q / 16 = ~624 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16_f_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option F fine-tuning completed successfully'
trap - EXIT
"
