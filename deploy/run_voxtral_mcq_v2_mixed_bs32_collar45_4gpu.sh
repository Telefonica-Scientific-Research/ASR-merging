#!/bin/bash
#SBATCH --job-name=voxtral_v2_col45
#SBATCH --output=./slurm_logs/voxtral_v2_mixed_bs32_collar45_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_mixed_bs32_collar45_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — v2 mixed | 124-file held-out eval | bs32 | collar=45s + remap"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_bs32_collar45_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
  --crop-from-question-refs \
  --remap-timestamps-after-crop \
  --crop-collar-seconds 45 \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_bs32_collar45_4gpu completed successfully'
