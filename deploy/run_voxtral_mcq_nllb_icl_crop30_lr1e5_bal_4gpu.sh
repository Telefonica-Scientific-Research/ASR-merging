#!/bin/bash
#SBATCH --job-name=voxtral_icl_l1_bal_4g
#SBATCH --output=./slurm_logs/voxtral_icl_l1_bal_4g_%j.out
#SBATCH --error=./slurm_logs/voxtral_icl_l1_bal_4g_%j.err
...
# R6-base + ICL rotation, no transcripts, lr=1e-5, balance-by-language
# ─────────────────────────────────────
# Identical training config to voxtral_mcq_nllb_transcript_4gpu_20260623_003920
# (LB 0.7382) with three changes:
#
#  1. No transcripts (--transcript-dir removed).
#  2. --random-crop-seconds 0: questions without timestamp references receive
#     the full audio instead of a random 300s crop.
#  3. --train-icl-audio-dir data/mlc26_task2/icl_examples: per-batch ICL
#     rotation using 20 pre-built 6-shot sets (4opt-A/B/C/D + 2opt-A/B).
#     Each batch prepends a randomly selected ICL set as masked demonstration
#     turns — model only supervised on the final answer.
#     Requires --use-chat-template-for-training (multi-turn template).
#
# The new audio-focus prefix is now part of every MCQ prompt in both training
# (via _build_mcq_prompt) and eval (via _render_mc_prompt / _build_jsonl_audio_mc_task).

set -euo pipefail

echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "ASR-merging Voxtral MCQ Train 4-GPU — R6 + ICL rotation (crop 30s, no transcripts)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_nllb_icl_crop30_lr1e5_bal_4gpu \
  --balance-by-language \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ R6 + ICL rotation lr=1e-5 balance-by-language 4-GPU training completed successfully'
