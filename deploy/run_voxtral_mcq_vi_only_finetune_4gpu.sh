#!/bin/bash
#SBATCH --job-name=voc_vi_only
#SBATCH --output=./slurm_logs/voc_vi_only_%j.out
#SBATCH --error=./slurm_logs/voc_vi_only_%j.err
...
# Per-language LoRA from OOTB Voxtral — VI (Vietnamese)
#
...
# Train: train_124files_vi_only.jsonl — 7 records, 210 questions
# Eval:  eval_26files_vi_only.jsonl  — 1 records, 30 questions (zero overlap)
# Steps: 210q / batch16 = ~13 steps/epoch; 10 epochs max
set -euo pipefail
echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "Voxtral MCQ — VI-only fine-tuning (OOTB base)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_124files_vi_only.jsonl \
  --eval-jsonl  data/mlc26_task2/eval_26files_vi_only.jsonl \
  --test-jsonl  data/mlc26_task2/eval_26files_vi_only.jsonl \
  ...
  --experiment-name voxtral_mcq_vi_only_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
  ...
  2>&1
echo 'VI-only fine-tuning completed successfully'
trap - EXIT
"
