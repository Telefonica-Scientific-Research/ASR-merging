#!/bin/bash
#SBATCH --job-name=voc_bs16h_ncn
#SBATCH --output=./slurm_logs/voc_bs16h_ncn_%j.out
#SBATCH --error=./slurm_logs/voc_bs16h_ncn_%j.err
...
# CL Phase 2 (bs16 halved-LR) — Option G: F + native
#
...
echo "Voxtral MCQ — CL bs16-halfLR Phase 2: Option G"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16h_g_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16-halfLR Phase 2 Option G completed successfully'
trap - EXIT
"
