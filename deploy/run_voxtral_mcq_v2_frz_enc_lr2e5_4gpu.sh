#!/bin/bash
#SBATCH --job-name=voxtral_v2_frz_enc_lr2e5
#SBATCH --output=./slurm_logs/voxtral_v2_frz_enc_lr2e5_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_frz_enc_lr2e5_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_frz_enc_lr2e5_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_frz_enc_lr2e5_4gpu completed successfully'
