#!/bin/bash
#SBATCH --job-name=voxtral_cl_nat_shuf
#SBATCH --output=./slurm_logs/voxtral_cl_nat_shuf_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_nat_shuf_%j.err
...
# Curriculum Learning Phase 2 — Option A_shuffled: native + option shuffling.
#
# Identical to Option A (EN×1 + native non-EN) but with answer options permuted
# for every non-EN record (226/263 records).  Uses the same deterministic
# per-question shuffle keyed on (path, question_id, options[0].text) as B_shuf.
#
# Motivation: the original dataset has severe label skew (A=34%, B=41%, C=23%,
# D=2%).  Shuffled copies break positional memorization — the model must reason
# about audio content rather than relying on position.  A_shuf tests whether
# this helps the best-performing A variant further.
#
# Comparison:
#   A          native           EN×1 + native non-EN             original option order
#   A_shuffled native_shuffled  EN×1 + native non-EN (shuffled)  anti-memorization  ← this script
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_native_shuffled.jsonl
#   - 263 records  (EN×1=37 unchanged + native non-EN×226 shuffled)
#   - 7,890 questions
#   - Answer dist: A=28% B=30% C=23% D=19% (vs original B=41%)
#   - EN share: 57%
#   - Steps (1 epoch, batch=32): ~247
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/.../final_model \
#   sbatch deploy/run_voxtral_mcq_cl_native_shuffled_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: native + option shuffling (A_shuffled)"
echo "=========================================="
...
# 7,890q / 32 = ~247 steps for 1 epoch (patience=3 → stops ~75 steps after best)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_native_shuffled.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_native_shuf_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 native shuffled fine-tuning completed successfully'
trap - EXIT
"
