#!/bin/bash
#SBATCH --job-name=voc_n150_frz_chat
#SBATCH --output=./slurm_logs/voc_n150_frz_chat_%j.out
#SBATCH --error=./slurm_logs/voc_n150_frz_chat_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_nllb150_frz_enc_conn_chat_tmpl_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
