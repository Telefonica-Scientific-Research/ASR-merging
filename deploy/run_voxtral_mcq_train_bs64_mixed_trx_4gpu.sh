#!/bin/bash
#SBATCH --job-name=voxtral_bs64_mixed_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_bs64_mixed_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_bs64_mixed_trx_4gpu_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_bs64_mixed_trx_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
