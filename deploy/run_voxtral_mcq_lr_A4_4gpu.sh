#!/bin/bash
#SBATCH --job-name=voxtral_mcq_lr_A4
#SBATCH --output=./slurm_logs/voxtral_mcq_lr_A4_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_lr_A4_%j.err
...
echo "ASR-merging Voxtral MCQ LR A4 enc=5e-5 llm=1e-4 4-GPU"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --eval-batch-size 1 \
  --grad-accum-steps 4 \
  --encoder-connector-learning-rate 5e-5 \
  --llm-learning-rate 1e-4 \
  --no-use-bf16 \
  --use-fp16 \
  --output-root experiments \
  --experiment-name voxtral_mcq_lr_A4 \
  --prompt-language en \
