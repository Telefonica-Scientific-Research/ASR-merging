#!/bin/bash
#SBATCH --job-name=voxtral_cl_nllb_cln_nat
#SBATCH --output=./slurm_logs/voxtral_cl_nllb_cln_nat_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_nllb_cln_nat_%j.err
...
# Curriculum Learning Phase 2 — Option G: NLLB clean + native same-language.
#
# Combines Option F's "safe" NLLB cross-lingual signal (EN-origin audio only,
# strictly EN → target_lang translation path) with the native same-language pairs
# from Option A.  Both signal types are present, and neither uses round-trip NLLB.
#
# Experiment matrix for CL Phase 2:
#   A  native              EN×1 + native non-EN                 same-lang only
#   B  nllb_up             EN×1 + NLLB×8 (all 150 files)        cross-lang, some round-trips
#   C  nllb_safe           EN×2 + NLLB×8 (all 150 files)        cross-lang, extra EN replay
#   D  full_mix            EN×1 + native + NLLB×8               both, 10% EN
#   E  full_mix_safe       EN×2 + native + NLLB×8               both, 19% EN
#   F  nllb_clean          EN×1 + NLLB_clean×8 (37 files only)  clean cross-lang
#   G  nllb_clean_native   EN×1 + NLLB_clean×8 + native         clean cross-lang + same-lang  ← this script
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_clean_native.jsonl
#   - 446 records  (EN-origin×1=37 + NLLB-clean×8=296 + native non-EN=113)
#   - 13,380 questions
#   - EN share: 8%
#   - Steps (1 epoch, batch=32): ~418  — compact, slightly larger than Option A
#
# Ablation value: compare with D (full_mix) to see if removing round-trip NLLB
# translations and keeping only clean EN-origin ones changes anything.
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/voxtral_mcq_nllb150_r32a64_duallr_chat_tmpl_4gpu_<ts>/final_model \
#   sbatch deploy/run_voxtral_mcq_cl_nllb_clean_native_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: NLLB clean + native (37 EN-origin files + 113 native)"
echo "=========================================="
...
# 13,380q / 32 = ~418 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_clean_native.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_nllb_clean_nat_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 NLLB clean + native fine-tuning completed successfully'
trap - EXIT
"
