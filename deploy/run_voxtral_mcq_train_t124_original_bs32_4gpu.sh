#!/bin/bash
#SBATCH --job-name=voxtral_t124_original_bs32
#SBATCH --output=./slurm_logs/voxtral_t124_original_bs32_%j.out
#SBATCH --error=./slurm_logs/voxtral_t124_original_bs32_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — original 124-file split, challenge-repr eval"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_124files_original.jsonl \
  ...
  --experiment-name voxtral_mcq_t124_original_bs32_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ original 124-file 4-GPU training completed successfully'
