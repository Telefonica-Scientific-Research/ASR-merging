#!/bin/bash
#SBATCH --job-name=voxtral_noen_mix_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_noen_mix_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_noen_mix_trx_4gpu_%j.err
...
# R6-base + no-English mixed dataset (original + NLLB) + transcript augmentation
# ────────────────────────────────────────────────────────────────────
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — R6-base + no-English mixed dataset + transcript"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_noen_mixed_transcript_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
