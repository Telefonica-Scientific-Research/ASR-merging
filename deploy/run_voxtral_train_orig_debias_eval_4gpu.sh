#!/bin/bash
#SBATCH --job-name=voxtral_ootb_train_orig_eval
#SBATCH --output=./slurm_logs/voxtral_ootb_train_orig_eval_%j.out
#SBATCH --error=./slurm_logs/voxtral_ootb_train_orig_eval_%j.err
...
#
# Usage:
#   sbatch deploy/run_voxtral_train_orig_debias_eval_4gpu.sh
#
...
#   OUTPUT_DIR=experiments/voxtral_ootb_train_orig_debias  (default)
#   MODEL_ID=mistralai/Voxtral-Mini-3B-2507                    (default) base model
#
# Output:
#   ${OUTPUT_DIR}/train_orig_hyp_{crop|full}[_debias].txt
#   ${OUTPUT_DIR}/train_orig_hyp_{crop|full}[_debias]_detail.jsonl   (when DEBIAS_CYCLIC=1)
...
OUTPUT_DIR="${OUTPUT_DIR:-experiments/voxtral_ootb_train_orig_debias}"

# Nested balanced-training shards (built once from the balanced full file, round-robin by row).
TRAIN_SHARD_REL="data/mlc26_task2/train_orig_eval_shards"
...
echo "=========================================="
echo "MareNostrum 5 - Voxtral OOTB eval on ORIGINAL (UNBALANCED) FULL TRAIN data (4-GPU, NO adapter, WITH gold)"
...
echo "Shard source  : ${TRAIN_SHARD_REL} (nested, original skewed A=34% B=40% C=23% D=2%)"
...
for i in $(seq 0 $((NUM_SHARDS - 1))); do
  ...
  SHARD_OUTPUT_REL="${EXPERIMENT_DIR}/train_orig_${EVAL_MODE_TAG}_${SHARD_TAG}"
  SHARD_OUTPUT_ABS="${REPO_HOST}/${SHARD_OUTPUT_REL}"
  SHARD_LOG="${SLURM_SUBMIT_DIR}/slurm_logs/voxtral_ootb_train_orig_eval_shard${i}_${SLURM_JOB_ID}.log"
...
FINAL_HYP="${REPO_HOST}/${EXPERIMENT_DIR}/train_orig_hyp_${EVAL_MODE_TAG}.txt"
echo ""
...
if [[ "${DEBIAS_CYCLIC}" == "1" ]]; then
  FINAL_DETAIL="${REPO_HOST}/${EXPERIMENT_DIR}/train_orig_hyp_${EVAL_MODE_TAG}_detail.jsonl"
  DETAIL_FILES=()
  for i in $(seq 0 $((NUM_SHARDS - 1))); do
