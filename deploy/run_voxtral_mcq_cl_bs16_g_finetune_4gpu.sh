#!/bin/bash
#SBATCH --job-name=voc_bs16_nlcn
#SBATCH --output=./slurm_logs/voc_bs16_nlcn_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_nlcn_%j.err
...
# CL Phase 2 (bs16 base) — Option G: F + native (446 rec, 13,380q)
#
...
#   - 446 records, 13380 questions, EN share 8%
#   - Steps (1 epoch, batch=16): ~836
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_g_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option G"
echo "=========================================="
...
# 13380q / 16 = ~836 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16_g_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option G fine-tuning completed successfully'
trap - EXIT
"
