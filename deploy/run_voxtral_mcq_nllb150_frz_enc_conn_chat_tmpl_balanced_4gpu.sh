#!/bin/bash
#SBATCH --job-name=voc_n150_frz_bal
#SBATCH --output=./slurm_logs/voc_n150_frz_bal_%j.out
#SBATCH --error=./slurm_logs/voc_n150_frz_bal_%j.err
...
echo "ASR-merging Voxtral MCQ — 150-file NLLB, freeze enc+conn, chat_tmpl, bs=16 (R6_base variant)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_nllb150_frz_enc_conn_chat_tmpl_balanced_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ nllb150 frz-enc-conn chat_tmpl bs=16 training completed successfully'
