#!/bin/bash
#SBATCH --job-name=voc_es_only
#SBATCH --output=./slurm_logs/voc_es_only_%j.out
#SBATCH --error=./slurm_logs/voc_es_only_%j.err
...
# Spanish-only fine-tuning from OOTB Voxtral (no base adapter)
#
...
# Train: train_124files_es_only.jsonl — 8 records (Spanish + Spanish_Mexico), 240q
# Eval:  eval_26files_es_only.jsonl  — 2 held-out records, 60q (zero overlap with train)
# Steps: 240q / batch16 = 15 steps/epoch; 10 epochs max → 150 steps total
# Eval every 3 steps to get frequent signal given the small dataset

set -euo pipefail

echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "Voxtral MCQ — Spanish-only fine-tuning (OOTB base)"
echo "=========================================="
...
# Train: 8 records, 240 questions; batch=16 → 15 steps/epoch
# 10 epochs max, eval every 3 steps, patience=5 (no improvement after 5 evals → stop)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \\
  ...
  --model-mode baseline \\
  --train-jsonl data/mlc26_task2/train_124files_es_only.jsonl \\
  --eval-jsonl  data/mlc26_task2/eval_26files_es_only.jsonl \\
  --test-jsonl  data/mlc26_task2/eval_26files_es_only.jsonl \\
  ...
  --experiment-name voxtral_mcq_es_only_ft_4gpu \\
  --timestamped-exp-dir \\
  --prompt-language en \\
...
echo 'Spanish-only fine-tuning completed successfully'
trap - EXIT
"
