#!/bin/bash
#SBATCH --job-name=voc_bs16_native
#SBATCH --output=./slurm_logs/voc_bs16_native_%j.out
#SBATCH --error=./slurm_logs/voc_bs16_native_%j.err
...
# CL Phase 2 (bs16 base) — Option A: EN×1 + native non-EN (263 rec, 7,890q)
#
...
#
# Usage:
#   sbatch deploy/run_voxtral_mcq_cl_bs16_a_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL bs16 Phase 2: Option A"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16_a_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16 Phase 2 Option A fine-tuning completed successfully'
trap - EXIT
"
