#!/bin/bash
#SBATCH --job-name=voxtral_mcq_s0_4gpu
#SBATCH --output=./slurm_logs/voxtral_mcq_s0_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_s0_4gpu_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU (shard 0)"
echo "=========================================="
...
# MLC audio data — cache has hardcoded /mnt/mirror1/datasets/MLC-SLM_Workshop_2026/... paths
...
if not any((latest / name).exists() for name in tokenizer_candidates):
  raise SystemExit(
    'Cached snapshot has no tokenizer artifacts at ' + str(latest)
  )
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --mcq-cache-dir data/cache/voxtral_mcq/audio_shard_base_4200 \
  --cache-train-split train \
  --cache-eval-split dev \
  --do-train \
  --do-eval \
  --num-shards 1 \
  --shard-index 0 \
  ...
  --experiment-name voxtral_mcq_original_cache_s0_4gpu \
  --prompt-language en \
...
echo 'Voxtral MCQ 4-GPU training completed successfully'
