#!/bin/bash
#SBATCH --job-name=voxtral_128noen_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_128noen_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_128noen_trx_4gpu_%j.err
...
# ────────────────────────────────────────────────────────────────────────
# TRAIN: train_128files_noen_balanced.jsonl
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_128files_noen_balanced.jsonl \
  ...
  --experiment-name voxtral_mcq_128noen_transcript_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
