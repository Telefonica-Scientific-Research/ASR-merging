#!/bin/bash
#SBATCH --job-name=voxtral_cl_native
#SBATCH --output=./slurm_logs/voxtral_cl_native_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_native_%j.err
...
# Curriculum Learning Phase 2: multilingual native injection.
#
# Fine-tunes a trained EN-only adapter for 1 epoch on the combined
# EN + native multilingual dataset (263 records, 7,890q, 43% native).
# Starts from a pre-trained adapter (e.g. Round 6 final_model) at halved LR
# to inject multilingual signal without catastrophic forgetting of English.
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/voxtral_mcq_nllb150_r32a64_duallr_chat_tmpl_4gpu_<ts>/final_model \
#   sbatch deploy/run_voxtral_mcq_cl_native_finetune_4gpu.sh
#
# Config vs Round 6:
#   enc_conn LR : 1e-5  →  5e-6  (halved — prevents over-writing EN representations)
#   llm LR      : 5e-5  →  2e-5  (halved)
#   epochs      : 3     →  1     (single injection pass)
#   patience    : 5     →  3     (1 epoch = ~246 steps, so 3×25=75 steps patience is fine)
#   train data  : EN-only (4500q) → EN + 12 native langs (7890q)
#   base adapter: none  →  BASE_ADAPTER_PATH (start from EN checkpoint)
...
echo "Voxtral MCQ — CL Phase 2: native multilingual injection"
echo "=========================================="
...
# 7890q / 32 = ~247 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_native_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 native multilingual fine-tuning completed successfully'
trap - EXIT
"
