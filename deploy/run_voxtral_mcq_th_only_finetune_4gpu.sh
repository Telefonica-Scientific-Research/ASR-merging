#!/bin/bash
#SBATCH --job-name=voc_th_only
#SBATCH --output=./slurm_logs/voc_th_only_%j.out
#SBATCH --error=./slurm_logs/voc_th_only_%j.err
...
# Per-language LoRA from OOTB Voxtral — TH (Thai)
#
...
# Train: train_124files_th_only.jsonl — 4 records, 120 questions
# Eval:  eval_26files_th_only.jsonl  — 1 records, 30 questions (zero overlap)
...
echo "Voxtral MCQ — TH-only fine-tuning (OOTB base)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_124files_th_only.jsonl \
  --eval-jsonl  data/mlc26_task2/eval_26files_th_only.jsonl \
  --test-jsonl  data/mlc26_task2/eval_26files_th_only.jsonl \
  ...
  --experiment-name voxtral_mcq_th_only_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'TH-only fine-tuning completed successfully'
trap - EXIT
"
