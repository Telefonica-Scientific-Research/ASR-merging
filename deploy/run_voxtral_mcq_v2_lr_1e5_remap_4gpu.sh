#!/bin/bash
#SBATCH --job-name=voxtral_v2r_lr_1e5
#SBATCH --output=./slurm_logs/voxtral_v2_lr_1e5_remap_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_lr_1e5_remap_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_v2_mixed_lr_1e5_remap_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_mixed_lr_1e5_4gpu remap completed successfully'
