#!/bin/bash
#SBATCH --job-name=voxtral_v2_nq_frz_chat
#SBATCH --output=./slurm_logs/voxtral_v2_nq_frz_chat_%j.out
#SBATCH --error=./slurm_logs/voxtral_v2_nq_frz_chat_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU"
echo "v2 nllb | freeze-encoder-connector | chat-template | collar30 | no remap"
echo "Data: train_124files_nllb.jsonl (EN+EN original + NLLB cross-lang, 3720q, 0% same-lang)"
echo "Rationale: all-English questions align with challenge inference strategy"
echo "           (challenge questions will be translated to EN before inference)"
echo "=========================================="
...
# Data: train_129files_nllb_qwen3.jsonl — NLLB English (all 150 files, 1 per language in test)
#   Train: 129 files × 30q × 2 translations = 7740q (NLLB + Qwen3 EN, EN audio appears twice)
#   Test:  21 files × 30q = 630q,  all English, 1 audio per language group (21 groups)
# Freeze: encoder + connector → LLM-only LoRA (r=16, alpha=32)
# Format: --use-chat-template-for-training (question inside [INST], no [TRANSCRIBE])
# Collar: 30s (optimal from sweep); no remap (remap consistently hurts in P1 vs P1v2)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_129files_nllb_qwen3.jsonl \
  --eval-jsonl data/mlc26_task2/eval_21files_en_only.jsonl \
  --test-jsonl data/mlc26_task2/eval_21files_en_only.jsonl \
  ...
  --experiment-name voxtral_mcq_v2_nllb_qwen3_frz_enc_conn_chat_tmpl_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ voxtral_mcq_v2_nllb_qwen3_frz_enc_conn_chat_tmpl_4gpu completed successfully'
