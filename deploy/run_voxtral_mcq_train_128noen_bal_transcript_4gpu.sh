#!/bin/bash
#SBATCH --job-name=voxtral_128noen_bal_trx_4gpu
#SBATCH --output=./slurm_logs/voxtral_128noen_bal_trx_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_128noen_bal_trx_4gpu_%j.err
...
echo "ASR-merging Voxtral MCQ Train 4-GPU — R6-base + 128-file non-overlap mixed + transcript"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_128_balanced_noen.jsonl \
  --eval-jsonl data/mlc26_task2/eval_21files_mixed_balanced.jsonl \
  ...
  --experiment-name voxtral_mcq_128noen_bal_transcript_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ 128-file non-overlap mixed + transcript 4-GPU training completed successfully'
