#!/bin/bash
#SBATCH --job-name=voxtral_fzo_l1_bal12_4g
#SBATCH --output=./slurm_logs/voxtral_fzo_l1_bal12_4g_%j.out
#SBATCH --error=./slurm_logs/voxtral_fzo_l1_bal12_4g_%j.err
...
# R6-base + ICL rotation, no transcripts, frozen encoder+connector, lr=1e-5, balance-by-language
# ────────────────────────────────────────────────────────────────
# Same as run_voxtral_mcq_nllb_icl_crop30_4gpu.sh but with
# --freeze-encoder: freeze audio encoder only; connector + LLM remain trainable
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_nllb_icl_crop30_frzenc_only_lr1e5_bal_12ep_4gpu \
  --balance-by-language \
  --freeze-encoder \
  --timestamped-exp-dir \
  --prompt-language en \
  ...
  --resume-from-checkpoint /opt/ASR-merging/experiments/voxtral_mcq_nllb_icl_crop30_frzenc_only_lr1e5_bal_4gpu_20260628_010215/checkpoint-150 \
  2>&1

echo 'Voxtral MCQ R6 + ICL rotation (frozen enc+conn) lr=1e-5 balance-by-language 12ep resume 4-GPU training completed successfully'
