#!/bin/bash
#SBATCH --job-name=voxtral_mcq_split_4gpu
#SBATCH --output=./slurm_logs/voxtral_mcq_split_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_mcq_split_4gpu_%j.err
...
# Train: 129 audio files (3870 questions).
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_train_audio_split.jsonl \
  ...
  --experiment-name voxtral_mcq_audio_split_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
