#!/bin/bash
#SBATCH --job-name=voxtral_bs64_128mixed_bal_mildw_4gpu
#SBATCH --output=./slurm_logs/voxtral_bs64_128mixed_bal_mildw_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_bs64_128mixed_bal_mildw_4gpu_%j.err
...
export TORCH_EXT_DIR="/gpfs/scratch/ehpc494/jls/torch_extensions"
export MLC_DATA_DIR="/gpfs/scratch/ehpc494/jls/datasets"
RUN_TAG="${SLURM_JOB_ID:-manual}_$(date +%Y%m%d_%H%M%S)"
RUN_ROOT="/gpfs/scratch/ehpc494/jls/asr_merging_runs/${RUN_TAG}"
...
# Effective batch size: 4 GPUs x 1 per-GPU batch x 16 grad_accum = 64
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_128_balanced_mixed_mildw.jsonl \
  ...
  --experiment-name voxtral_mcq_bs64_128mixed_bal_mildw_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
