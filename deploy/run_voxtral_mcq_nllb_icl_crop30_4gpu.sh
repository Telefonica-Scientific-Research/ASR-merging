#!/bin/bash
#SBATCH --job-name=voxtral_nllb_icl_4gpu
#SBATCH --output=./slurm_logs/voxtral_nllb_icl_4gpu_%j.out
#SBATCH --error=./slurm_logs/voxtral_nllb_icl_4gpu_%j.err
...
# R6-base + ICL rotation, no transcripts
# ─────────────────────────────────────
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_nllb_icl_crop30_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ R6 + ICL rotation 4-GPU training completed successfully'
