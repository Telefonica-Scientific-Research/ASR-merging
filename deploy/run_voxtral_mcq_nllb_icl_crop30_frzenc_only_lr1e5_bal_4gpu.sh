#!/bin/bash
#SBATCH --job-name=voxtral_fz_l1_bal_4g
#SBATCH --output=./slurm_logs/voxtral_fz_l1_bal_4g_%j.out
#SBATCH --error=./slurm_logs/voxtral_fz_l1_bal_4g_%j.err
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_nllb_icl_crop30_frzenc_only_lr1e5_bal_4gpu \
  --balance-by-language \
  --freeze-encoder \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'Voxtral MCQ R6 + ICL rotation (frozen enc+conn) lr=1e-5 balance-by-language 4-GPU training completed successfully'
