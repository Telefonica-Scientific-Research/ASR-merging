#!/bin/bash
#SBATCH --job-name=voc_n150_bal_all
#SBATCH --output=./slurm_logs/voc_n150_bal_all_%j.out
#SBATCH --error=./slurm_logs/voc_n150_bal_all_%j.err
...
# Round 6 — Best-of-all-ablations training:
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_successed_opensource_en_translated_balanced.jsonl \
  ...
  --experiment-name voxtral_mcq_nllb150_r32a64_duallr_chat_tmpl_balanced_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
