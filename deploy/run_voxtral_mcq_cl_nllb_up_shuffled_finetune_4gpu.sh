#!/bin/bash
#SBATCH --job-name=voxtral_cl_nllb_shuf
#SBATCH --output=./slurm_logs/voxtral_cl_nllb_shuf_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_nllb_shuf_%j.err
...
# Curriculum Learning Phase 2 — Option B_shuffled: NLLB upsample + option shuffling.
#
# Identical to Option B (EN×1 + NLLB×8) but with answer options permuted for
# every non-EN record.  Each language copy uses a deterministic per-question
# shuffle keyed on (question_id, language), so the correct answer label (A/B/C/D)
# differs across copies.
#
# Motivation: without shuffling, the same audio+question appears 8× with the
# answer always at the same position (e.g. always "B"), letting the LLM memorize
# the answer letter rather than reasoning about the multilingual question.
# After shuffling each language copy gets ~25% per option, breaking positional
# memorization.
#
# Side-effect benefit: the original EN data had severe label skew (A=34%, B=41%,
# C=23%, D=2%).  Shuffled copies are balanced (~25% per option), which also
# reduces positional bias learned from the data.
#
# Experiment matrix for CL Phase 2 (shuffled variants):
#   B          nllb_up          EN×1 + NLLB×8                original option order
#   B_shuffled nllb_up_shuffled EN×1 + NLLB×8 (shuffled)    anti-memorization  ← this script
#   E          full_mix_safe    EN×2 + native + NLLB×8       original option order
#   E_shuffled fms_shuffled     EN×2 + native + NLLB×8 (sh.) anti-memorization
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_up_shuffled.jsonl
#   - 1,350 records  (EN×1=150 unchanged + NLLB×8=1200 shuffled)
#   - 40,500 questions
#   - EN share: 11%
#   - Steps (1 epoch, batch=32): ~1,265
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/.../final_model \
#   sbatch deploy/run_voxtral_mcq_cl_nllb_up_shuffled_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: NLLB upsample + option shuffling (B_shuffled)"
echo "=========================================="
...
# 40,500q / 32 = ~1,265 steps for 1 epoch
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_nllb_up_shuffled.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_nllb_up_shuf_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 NLLB upsample shuffled fine-tuning completed successfully'
trap - EXIT
"
