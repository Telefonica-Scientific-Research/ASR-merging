#!/bin/bash
#SBATCH --job-name=voxtral_cl_full_mix
#SBATCH --output=./slurm_logs/voxtral_cl_full_mix_%j.out
#SBATCH --error=./slurm_logs/voxtral_cl_full_mix_%j.err
...
# Curriculum Learning Phase 2 — Option D: full mix (EN×1 + native non-EN + NLLB×8).
#
# Combines both types of multilingual signal:
#   - Same-language pairs  (native audio + native questions)  from Option A
#   - Cross-language pairs (EN audio   + NLLB questions)      from Option B
# This is the "best of both worlds" dataset without extra EN replay.
#
# Experiment matrix for CL Phase 2:
#   A  native         EN×1 + native non-EN                  same-lang only
#   B  nllb_up        EN×1 + NLLB×8                         cross-lang only, 11% EN
#   C  nllb_safe      EN×2 + NLLB×8                         cross-lang only, 20% EN
#   D  full_mix       EN×1 + native non-EN + NLLB×8         both,            10% EN  ← this script
#   E  full_mix_safe  EN×2 + native non-EN + NLLB×8         both,            19% EN
#
# Dataset: data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_full_mix.jsonl
#   - 1,463 records  (EN×1=150 + native=113 + NLLB×8=1200)
#   - 43,890 questions
#   - EN share: 10%  (low — same forgetting risk as Option B)
#   - Steps (1 epoch, batch=32): ~1,371
#
# Usage:
#   BASE_ADAPTER_PATH=experiments/voxtral_mcq_nllb150_r32a64_duallr_chat_tmpl_4gpu_<ts>/final_model \
#   sbatch deploy/run_voxtral_mcq_cl_full_mix_finetune_4gpu.sh
...
echo "Voxtral MCQ — CL Phase 2: full mix (EN×1 + native + NLLB×8)"
echo "=========================================="
...
# 43,890q / 32 = ~1,371 steps for 1 epoch (early stopping with patience=5 expected)
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --train-jsonl data/mlc26_task2/mlcslm_2nd_dev_qa_150files_cl_full_mix.jsonl \
  ...
  --experiment-name voxtral_mcq_cl_full_mix_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL Phase 2 full mix fine-tuning completed successfully'
trap - EXIT
"
