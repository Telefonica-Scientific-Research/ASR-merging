#!/bin/bash
#SBATCH --job-name=voc_ja_only
#SBATCH --output=./slurm_logs/voc_ja_only_%j.out
#SBATCH --error=./slurm_logs/voc_ja_only_%j.err
...
# Per-language LoRA from OOTB Voxtral — JA (Japanese)
#
...
# Train: train_124files_ja_only.jsonl — 10 records, 300 questions
# Eval:  eval_26files_ja_only.jsonl  — 1 records, 30 questions (zero overlap)
# Steps: 300q / batch16 = ~18 steps/epoch; 10 epochs max
set -euo pipefail
echo "=========================================="
echo "MareNostrum 5 - Acceleration Partition"
echo "Voxtral MCQ — JA-only fine-tuning (OOTB base)"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --model-mode baseline \
  --train-jsonl data/mlc26_task2/train_124files_ja_only.jsonl \
  --eval-jsonl  data/mlc26_task2/eval_26files_ja_only.jsonl \
  --test-jsonl  data/mlc26_task2/eval_26files_ja_only.jsonl \
  ...
  --experiment-name voxtral_mcq_ja_only_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
  ...
  2>&1
echo 'JA-only fine-tuning completed successfully'
trap - EXIT
"
