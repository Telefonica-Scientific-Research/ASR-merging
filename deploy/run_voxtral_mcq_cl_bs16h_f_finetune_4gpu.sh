#!/bin/bash
#SBATCH --job-name=voc_bs16h_ncl
#SBATCH --output=./slurm_logs/voc_bs16h_ncl_%j.out
#SBATCH --error=./slurm_logs/voc_bs16h_ncl_%j.err
...
# CL Phase 2 (bs16 halved-LR) — Option F: EN-origin×1 + NLLB-clean×8
#
...
echo "Voxtral MCQ — CL bs16-halfLR Phase 2: Option F"
echo "=========================================="
...
torchrun --standalone --nproc_per_node=4 --master_port \${MASTER_PORT} \
  ...
  --experiment-name voxtral_mcq_cl_bs16h_f_ft_4gpu \
  --timestamped-exp-dir \
  --prompt-language en \
...
echo 'CL bs16-halfLR Phase 2 Option F completed successfully'
trap - EXIT
"
